...
class Router(object):
    def __init__(self, net):
        ...
        self.queue = []
        self.drm_forwarding_table = []

        # establish forwarding table using router interface
        ...
        # establish forwarding table using router provided .txt
        ...
        if fp is not None:
            for line in fp:
                ...
                self.forwarding_table.append(entry)
                self.drm_forwarding_table.append(entry)
            fp.close()
    ...
    def get_longest_prefix(self, ip):
        ...
        '''
        entries = []
        for drm_entry in self.drm_forwarding_table:
            if ip in drm_entry.network:
                entries.append(drm_entry)
    ...
    def router_main(self):    
        ...
        while True:
            ...
            if gotpkt:
                ...
                # start receiving dynamic routing message pkts first
                if pkt.has_header(DynamicRoutingMessage):
                    entry_exist = False
                    drm = pkt.get_header(DynamicRoutingMessage)
                    for entry in self.drm_forwarding_table:
                        # if a same entry already exists in the table, update next hop
                        if entry.network == IPv4Network(str(drm.advertised_prefix) + '/' + str(drm.advertised_mask), False):
                            entry_exist = True
                            entry.next_hop = drm.next_hop
                            # FIXME: write-up also required us to update interface name
                            entry.intf_name = dev
                    
                    if entry_exist == False:
                        # else do FIFO until there's space and add new entry
                        while len(self.drm_forwarding_table) >= 5:
                            self.drm_forwarding_table.pop(0)
                        new_entry = ForwardingTableEntry(IPv4Network(str(drm.advertised_prefix) + '/' + str(drm.advertised_mask), False), drm.next_hop, dev)
                        self.drm_forwarding_table.append(new_entry)
                ...
                if pkt.has_header(IPv4):
                    ...
                    # get longest prefix matched dst entry from static and dynamic tables 
